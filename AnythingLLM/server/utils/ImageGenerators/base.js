...
class BaseImageGenerator {
  ...
  /**
   * Sanitiza el prompt para prevenir que modelos generativos introduzcan marcas de agua,
   * textos o firmas artificiales, bajo principios de gobernanza /ECC.
   * @param {string} prompt
   * @returns {string}
   */
  _sanitizePrompt(prompt) {
    ...
    if (!/no watermark/i.test(clean)) {
      clean = `${clean}, no watermark, no text, clean image`;
    }
    return clean;
  }
  ...
  async generateImage({ prompt, size, signal }) {
    ...
    const result = await this.requestImage(cleanPrompt, imageSize, signal);
