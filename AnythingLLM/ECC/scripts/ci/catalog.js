...
const WORKING_CONTEXT_PATH = path.join(ROOT, 'WORKING-CONTEXT.md');
...
function parseSoulExpectations(soulContent) {
  const match = soulContent.match(/with\s+(\d+)\s+specialized agents,\s+(\d+)\s+skills,\s+(\d+)\s+commands/i);
  if (!match) {
    throw new Error('SOUL.md is missing the catalog summary line');
  }

  return [
    { category: 'agents', mode: 'exact', expected: Number(match[1]), source: 'SOUL.md summary' },
    { category: 'skills', mode: 'exact', expected: Number(match[2]), source: 'SOUL.md summary' },
    { category: 'commands', mode: 'exact', expected: Number(match[3]), source: 'SOUL.md summary' }
  ];
}

function parseWorkingContextExpectations(wcContent) {
  const match = wcContent.match(/Public catalog truth is `(\d+)` agents, `(\d+)` commands, and `(\d+)` skills/i);
  if (!match) {
    throw new Error('WORKING-CONTEXT.md is missing the catalog truth line');
  }

  return [
    { category: 'agents', mode: 'exact', expected: Number(match[1]), source: 'WORKING-CONTEXT.md current truth' },
    { category: 'commands', mode: 'exact', expected: Number(match[2]), source: 'WORKING-CONTEXT.md current truth' },
    { category: 'skills', mode: 'exact', expected: Number(match[3]), source: 'WORKING-CONTEXT.md current truth' }
  ];
}
...
function syncSoul(content, catalog) {
...
function syncWorkingContext(content, catalog) {
  return replaceOrThrow(
    content,
    /(Public catalog truth is `)(\d+)(` agents, `)(\d+)(` commands, and `)(\d+)(` skills)/i,
    (_, prefix, __, agentsSuffix, ___, commandsSuffix, ____, skillsSuffix) =>
      `${prefix}${catalog.agents.count}${agentsSuffix}${catalog.commands.count}${commandsSuffix}${catalog.skills.count}${skillsSuffix}`,
...
function createDocumentSpecs(paths = {}) {
  const {
    ...
    soulPath = SOUL_PATH,
    workingContextPath = WORKING_CONTEXT_PATH,
  } = paths;
  ...
  return [
    ...
    {
      filePath: soulPath,
      parseExpectations: parseSoulExpectations,
      syncContent: syncSoul,
    },
    {
      filePath: workingContextPath,
      parseExpectations: parseWorkingContextExpectations,
      syncContent: syncWorkingContext,
    },
  ];
}
...
function createDocumentSpecsForRoot(root) {
  return createDocumentSpecs({
    ...
    workingContextPath: path.join(root, 'WORKING-CONTEXT.md'),
  });
}
...
module.exports = {
  ...
  runCatalogCheck,
  parseSoulExpectations,
  parseWorkingContextExpectations,
