...
async function main() {
  ...
  // Clear shared compact flag so suggest-compact can fire again next phase
  const compactFlagPath = path.join(sessionsDir, '.compact-scheduled');
  try {
    if (fs.existsSync(compactFlagPath)) {
      fs.rmSync(compactFlagPath, { force: true });
      log('[PreCompact] Cleared compact-scheduled flag');
    }
  } catch (err) {
    log(`[PreCompact] Could not clear compact flag: ${err.message}`);
  }
