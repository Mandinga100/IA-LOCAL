...
## Over-orchestration Prevention

- Do not call planner or architect for tasks with ≤4 files, 1 layer, known root cause.
- Do not open deep exploration if root cause is already confirmed.
- Do not compact if task did not exceed the adaptive context threshold.
- Do not run full quality gates on single-file edits; use incremental validation.
- If escalating a task that meets direct-resolution criteria, append a one-line justification.
