...
Este documento detalla la arquitectura desacoplada de 5 capas implementada en el módulo [`core/`](../Base/core), su integración con el API Gateway [`servidor_api.py`](../Base/servidor_api.py), la telemetría de hardware 360° y la compatibilidad universal con **AnythingLLM** y **Open WebUI** mediante endpoints compatibles con OpenAI (`/v1`).
...
```
                         [ AnythingLLM Desktop / Docker / Open WebUI ]
                                                │
                                                ▼ (HTTP OpenAI-Compatible: /v1/chat/completions)
[ Frontend Web 360° ] ────────► [ API Gateway: servidor_api.py (FastAPI) ]
                                                │
                ┌───────────────────────────────┴───────────────────────────────┐
                ...
                │  [Capa 1: connector.py]   ──► Ollama Multimodal (FlashAttn)   │
                │  [Seguridad: ecc_guard.py]──► Verificación SHA-256 CEO        │
                └───────────────────────────────┬───────────────────────────────┘
                                                │ (HTTP Asíncrono / JSON)
                                         [ Ollama Local ]
                                                │
...
- **Aceleración en Producción (Blackwell 24 GB GDDR7 ECC):**
  ...
  - Cuantización de KV-Cache (`kv_cache_type: q8_0`), reduciendo el consumo de VRAM en contextos masivos (32K/65K) en más de un 60% sin degradación de precisión.
...
| **`chat_ui`** | `qwen2.5-coder:32b` | `qwen2.5:3b` | 16,384 | 2,048 | AnythingLLM / Open WebUI, diálogo técnico |
...
- **Control de Concurrencia:** Inferencia serializada con `asyncio.Semaphore(1)` o slots acotados para prevenir desbordamientos de memoria (`CUDA OOM`).
...
### Capa de Seguridad Transversal: Guardia Criptográfica (`core/ecc_guard.py`)
- **Responsabilidad:** Protege las carpetas inmutables del arnés (`ECC/` y `ai-harness/ecc/`).
- **Desafío de Identidad:** Aplica verificación unidireccional SHA-256 autorizada exclusivamente al CEO sin almacenar nombres en texto claro en repositorios ni logs.

---

## 🌐 Endpoints Expuestos en `servidor_api.py`

### 1. Endpoints de Compatibilidad OpenAI (`/v1`) para AnythingLLM y Open WebUI
- **`GET /v1/models`**: Retorna los perfiles lógicos como modelos virtuales junto a cualquier modelo físico en Ollama.
- **`POST /v1/chat/completions`**:
  - Acepta la especificación OpenAI (`model`, `messages`, `temperature`, `stream`).
  - Mapea el modelo al perfil correspondiente (`chat_ui`, `doc_main`, etc.).
...
- **`POST /api/procesar`**: Endpoint principal de documentos con soporte para el parámetro opcional `perfil`.

### 3. Endpoints de Telemetría 360° y Gestión Documental
- **`GET /api/telemetria/360`**: Métricas de hardware en tiempo real: VRAM usada/libre, temperatura GPU, carga CUDA %, memoria RAM y uso de CPU.
- **`GET /api/documentos`**: Lista histórica y estado de documentos procesados.
- **`GET /api/ver/{nombre}`**: Visor web interactivo para previsualizar documentos originales y corregidos.
- **`GET /api/descargar/{nombre}`**: Descarga física directa de los archivos compilados en `datos/salida_web/`.
- **`GET /api/modelos`**: Catálogo en tiempo real de modelos instalados en Ollama y estado de carga en VRAM.
- **`GET /api/health`**: Verificación de salud del servidor FastAPI y enlace con Ollama.
