...
- no new secrets, unsafe deserialization, unsafe `eval`/`exec`, expanded
  network egress, or expanded credential/filesystem access were introduced
  by the winning variant — run a lightweight security pass (`security-scan`
  or the project's `security-reviewer`) before promotion, not after;
- rollback is obvious;
- the change is encoded in source control or a durable runbook;
- the final summary includes exact commands and measurements.

The security check is non-negotiable for unattended or scheduled loops
(see `continuous-agent-loop`'s "Unattended / Scheduled Runs" section):
a loop with Bash access iterating without a human in it will happily
promote a "faster" variant that got there by disabling a check, widening
a permission, or shelling out somewhere it shouldn't. Correctness tests
alone do not catch that class of regression.
