...
async function main() {
  ...
  // Shared flag with pre-compact.js — skip if compact already scheduled
  const sessionsDir = getSessionsDir();
  const compactFlag = path.join(sessionsDir, '.compact-scheduled');
  if (fs.existsSync(compactFlag)) {
    log('[StrategicCompact] Compact already scheduled — skipping');
    process.exit(0);
  }
  ...
  // Write shared flag so pre-compact can clear it when compaction actually runs
  if (messages.length > 0) {
    try {
      fs.writeFileSync(compactFlag, getDateTimeString(), 'utf-8');
    } catch (err) {
      log(`[StrategicCompact] Could not write compact flag: ${err.message}`);
    }
  }
