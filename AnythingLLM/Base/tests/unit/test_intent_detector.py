...
class TestIntentDetector:
    ...
    def test_ejecutar_exportacion_automatica_pdf(self, tmp_path: Path) -> None:
        ...
        assert "Opciones de Descarga" in res["bloque_markdown"]
        assert "<details" not in res["bloque_markdown"]
        assert "</details>" not in res["bloque_markdown"]
        assert "Ruta Local en Servidor" not in res["bloque_markdown"]
        assert "Copia en Escritorio" not in res["bloque_markdown"]

    def test_ejecutar_exportacion_rechazo_cancela_exportacion(self, tmp_path: Path) -> None:
        """Verifica que un mensaje de rechazo de la IA NO genere archivo ni bloque de descarga."""
        intent = ExportIntent(
            es_exportacion=True,
            formato=".pdf",
            nombre_base="documento_rechazado"
        )
        texto_rechazo = (
            "Lo siento, pero como asistente de inteligencia artificial, no tengo la capacidad de analizar "
            "o corregir documentos. Sin embargo, puedo ayudarte a crear un nuevo documento de calidad. "
            "Por favor, proporciona el contenido del documento que deseas que se revise y corrija."
        )
        res = ejecutar_exportacion_automatica(
            intent=intent,
            texto_markdown=texto_rechazo,
            salida_dir=tmp_path
        )
        assert res["exito"] is False
        assert res["motivo"] == "rechazo_modelo"
        assert res["bloque_markdown"] == ""
        archivo_candidato = tmp_path / "documento_rechazado.pdf"
        assert not archivo_candidato.exists()
