# Integración AnythingLLM Desktop / Docker, Protocolo Zero-Chatter y Preservación Visual Pixel-Perfect

**Documento:** `docs/integracion_anythingllm_mcp_y_pureza.md`  
**Estado:** PRODUCCIÓN / OPERATIVO  
**Gobernanza:** SDP-U / Arquitectura /ECC  
**Entorno Operativo:** Windows 10 Pro 64-bit, GNU/Linux, Ollama Local (Qwen 2.5 3B / 32B), AnythingLLM, Python 3.13.14  

---

## 1. Resumen Ejecutivo

La **Plataforma IA Local** opera bajo un modelo desacoplado donde **AnythingLLM** (en sus versiones Desktop GUI y contenedor Docker Multi-Usuario) asume el rol de interfaz de interacción para el usuario final y los equipos de trabajo.

A través de un **Action-Aware Gateway** compatible con la especificación de OpenAI (`/v1/chat/completions`), un **Servidor MCP Oficial** (`mcp_server.py`) potenciado por el arnés `/ECC` y motores deterministas de **Pureza Documental (Zero-Chatter)** y **Preservación Visual Pixel-Perfect**, el sistema procesa, corrige y genera físicamente documentos (`.pdf`, `.docx`, `.odt`, `.rtf`, `.html`, `.md`, `.txt`, `.csv`) garantizando cero preámbulos conversacionales ni degradación gráfica.

```
┌──────────────────────────────────────────────────────────────────────────────────┐
│                   ANYTHINGLLM DESKTOP / DOCKER MULTI-USER                        │
│  - Workspaces Departamentales (Doc, Finanzas, Presentaciones, Scripts)           │
│  - Proveedor LLM: OpenAI Compatible (http://127.0.0.1:8000/v1)                   │
│  - MCP Client: plataforma_ia_local (Base/mcp_server.py stdio)                    │
└────────────────────────────────────────┬─────────────────────────────────────────┘
                                         │ HTTP /v1/chat/completions (SSE Stream / Batch)
                                         ▼
┌──────────────────────────────────────────────────────────────────────────────────┐
│                   ACTION-AWARE GATEWAY (Base/servidor_api.py)                    │
...
Al procesar cualquier solicitud de exportación desde AnythingLLM o el frontend visual 360°, el usuario recibe 4 vías simultáneas de acceso:
...
```json
{
  "mcpServers": {
    "plataforma_ia_local": {
      "command": "C:\\Users\\mandi\\Documents\\Proyectos\\Plataforma IA local\\AnythingLLM\\Base\\.venv\\Scripts\\python.exe",
      "args": [
        "C:\\Users\\mandi\\Documents\\Proyectos\\Plataforma IA local\\AnythingLLM\\Base\\mcp_server.py"
      ]
    }
  }
}
...
El campo `openAiPrompt` de la tabla `workspaces` fue configurado con las directivas de pureza documental y gobernanza /ECC para impedir que el LLM emita saludos o introducciones conversacionales.

---

## 7. Verificación y Cobertura de Pruebas

Toda la implementación cuenta con cobertura unitaria automatizada ejecutada con `pytest` dentro de la suite global de 176 pruebas:
