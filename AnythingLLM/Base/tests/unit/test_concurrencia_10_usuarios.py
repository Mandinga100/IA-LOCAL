...
def test_workspaces_configuracion_valida():
    ...
    posibles = [
        Path("ai-harness/ecc/workspaces"),
        Path("../ai-harness/ecc/workspaces"),
        Path(__file__).resolve().parents[3] / "ai-harness" / "ecc" / "workspaces",
    ]
    ruta_ws = next((p for p in posibles if p.exists()), Path("ai-harness/ecc/workspaces"))
