...
**Entorno Operativo:** Windows 10 Pro 64-bit / Linux, Docker Desktop / WSL2 / Docker Engine, Ollama Local (Qwen 2.5 3B), NVIDIA GTX 1650 (4 GB VRAM) / Blackwell 24GB  
...
5. **Automatización Multiplataforma Dual:** Scripts nativos de despliegue tanto para Windows PowerShell (`.ps1`) como para entornos Linux Bash (`.sh`).

---

## 2. Presupuesto Matemático de Hardware y VRAM (10 Usuarios)

### Especificaciones del Host (MVP)
...
Ubicado en la raíz de `Base/` y adaptado para el host:
```yaml
version: '3.8'
...
### 3.2. Scripts de Despliegue Automatizado

#### En Windows PowerShell:
```powershell
.\Base\scripts\desplegar_anythingllm_docker.ps1
```
...
### 3.3. Scripts de Optimización de Ollama

#### En Windows PowerShell:
```powershell
.\Base\scripts\optimizar_ollama_concurrencia.ps1
```

#### En Linux Bash:
```bash
chmod +x Base/scripts/optimizar_ollama_concurrencia.sh
...
Los esquemas JSON preconfigurados se ubican en `ai-harness/ecc/workspaces/` y en `Base/produccion/workspaces/`:
...
- **Cobertura total de la suite:** **176 pruebas automatizadas (100% VERDE)**.
