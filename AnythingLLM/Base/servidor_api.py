...
from core.pureza_documental import es_respuesta_de_rechazo, normalizar_mojibake
...
async def v1_models() -> JSONResponse:
    ...
    # 3. Modelos activos en llama-server si está conectado
    if await connector.is_llama_server_online():
        for ls_id in ("llama-server", "poc_llama_server/qwen2.5-3b.gguf", "qwen2.5-3b.gguf"):
            model_list.append({
                "id": ls_id,
                "object": "model",
                "created": now_ts,
                "owned_by": "llama.cpp-local",
                "permission": [],
                "root": ls_id,
                "parent": None
            })
...
async def v1_chat_completions(req: ChatCompletionRequest):
    ...
    # 1. Sanitizar el historial de conversación para eliminar negativas heredadas y mojibake
    messages_payload = []
    for m in req.messages:
        # Si es un mensaje previo del asistente y es un rechazo/disculpa, lo purgamos del historial
        # para evitar que el modelo alucine una negativa en cadena basada en su propio turno anterior
        if m.role == "assistant" and es_respuesta_de_rechazo(m.content):
            logger.info("Guardarraíl Historial: Mensaje de disculpa/rechazo previo del asistente purgado del contexto.")
            continue
        # Normalizar activamente mojibake y prevenir catalanismos inducidos por errores de codificación
        contenido_limpio = normalizar_mojibake(m.content)
        messages_payload.append({"role": m.role, "content": contenido_limpio})

    # Detección inteligente de contexto adjunto e intención de exportación
    tiene_contexto_adjunto = any("[CONTEXT" in m.get("content", "") for m in messages_payload)
    export_intent = detectar_intencion_exportacion(messages_payload)

    # Escalado Enterprise a qwen2.5:7b si está instalado y la tarea es documental (solo si llama-server no está acelerando)
    if not (await connector.is_llama_server_online()) and "qwen2.5:7b" in installed_names:
        if tiene_contexto_adjunto or (export_intent and export_intent.es_exportacion) or chosen_model in ("qwen2.5:3b", "qwen2.5:1.5b", "default"):
            if chosen_model != "qwen2.5:7b":
                logger.info(f"Escalado Enterprise: Elevando modelo '{chosen_model}' -> 'qwen2.5:7b' para máxima precisión ortotipográfica y arquitectónica.")
                chosen_model = "qwen2.5:7b"
                profile = resolver_perfil("qwen2.5:7b")

    chat_id = f"chatcmpl-{int(time.time() * 1000)}"
    created_ts = int(time.time())

    # 2. Inyección inteligente de capacidad documental para AnythingLLM:
    if tiene_contexto_adjunto:
        directiva_adjuntos = (
            "[DIRECTIVA VINCULANTE DEL SISTEMA - ESTÁNDAR SENIOR ENTERPRISE]\n"
            "1. ACCESO A ARCHIVOS: El texto íntegro del documento adjuntado por el usuario se encuentra transcrito en los bloques 'Context:' ([CONTEXT 0], etc.). TIENES ACCESO TOTAL a él. NUNCA respondas que no puedes leer, procesar o abrir archivos adjuntos, ni pidas al usuario que vuelva a pegar el contenido.\n"
            "2. PROHIBICIÓN DE CHATTER Y SIMULACIÓN DE EXPORTACIÓN: La compilación física del archivo descargable (.pdf, .docx) la ejecuta silenciosamente el servidor backend. NUNCA escribas frases como 'Generando archivo...', '[El archivo se generará automáticamente...]', 'Archivo generado:', '[Archivo adjunto...]', 'Este resumen proporciona...', ni agregues notas al pie como 'Nota: Los campos [pendiente]...'. Escribe ÚNICAMENTE el contenido textual limpio y estructurado del documento.\n"
            "3. IDIOMA ESTRICTO: El documento debe redactarse 100% en ESPAÑOL TÉCNICO FORMAL. Queda estrictamente PROHIBIDO el uso de catalán, valenciano u otro idioma no solicitado.\n"
            "4. REHABILITACIÓN ENTERPRISE: Transforma todo contenido irónico, satírico, informal o roto (ejemplos: 'reza para que funcione', 'llora un poco', 'o algo así', 'no estamos seguros', 'no respondemos') en especificaciones y procedimientos de ingeniería de software corporativo profesionales y rigurosos.\n"
            "5. SINTAXIS Y CÓDIGO PERFECTO: Corrige todos los errores de sintaxis en los bloques de código (ej. cerrar paréntesis, llaves, comillas) para que sean 100% funcionales y compilables. Corrige diagramas de arquitectura para que reflejen un flujo Enterprise coherente.\n"
            "6. FIDELIDAD ABSOLUTA Y PUREZA DOCUMENTAL: Entrega el documento 100% fiel al compartido por el usuario y limitado estrictamente a su orden de iteración. No agregues preámbulos, saludos, disculpas, epílogos, ni campos inventados. Comienza directamente con el título o contenido correspondiente y completa todas las secciones sin truncar.\n\n"
        )
        has_system = False
        for msg in messages_payload:
            if msg.get("role") == "system":
                if "[DIRECTIVA VINCULANTE DEL SISTEMA" not in msg.get("content", ""):
                    msg["content"] = f"{directiva_adjuntos}\n{msg['content']}"
                has_system = True
                break
        if not has_system:
            messages_payload.insert(0, {"role": "system", "content": directiva_adjuntos})

    # Presupuesto dinámico de tokens para evitar truncamiento
    token_budget = max(req.max_tokens or 0, profile.num_predict or 4096, 4096)
        ...
        async def event_generator() -> AsyncGenerator[str, None]:
            ...
            try:
                ...
                if export_intent and export_intent.es_exportacion:
                    ...
                    if es_respuesta_de_rechazo(texto_generado):
                        logger.warning(
                            "Exportación omitida en streaming: la respuesta generada es un rechazo o disculpa del modelo."
                        )
                    else:
                        ...
                        if texto_exportar and not es_respuesta_de_rechazo(texto_exportar):
                            try:
    ...
    try:
        ...
        if export_intent and export_intent.es_exportacion:
            ...
            if es_respuesta_de_rechazo(texto_exportar):
                logger.warning("Exportación omitida en batch: la respuesta generada es un rechazo del modelo.")
            else:
